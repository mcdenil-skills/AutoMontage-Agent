...
- `--autotheme` больше не использует `node-vibrant`: ffmpeg отдаёт до 20 характерных кадров
  ...
  PNG-файлы (#33).
- Необязательный бенчмарк `examples/broll-benchmark` переведён на transformers 5.10.1 и
  huggingface-hub 1.33.0 согласованным набором: это закрывает четыре алерта transformers 4.57.6.
  Номера токенов SigLIP2 и выходы модели со случайными весами совпали с прежним окружением;
  прогон с настоящими весами не выполнялся (#32).

### Исправлено

- Тесты больше не оставляют в системной временной папке git-репозитории, проекты, папки тем,
  промежуточные видео и надгробия удаления: раньше каждый `npm test` добавлял около 40 записей.
  Новый `tests/test-temp-hygiene.test.js` прогоняет затронутые файлы в отдельной временной
  папке и падает, если после них что-то осталось (#31).
- `--autotheme` берёт характерные кадры из первых 2000 кадров видео (≈80 с при 25 fps): раньше
  ffmpeg повторял первый из них 20 раз, и при `--brandLock` меньше 1 цвет темы определяли
  только первые ~4 с ролика (#33).
...
- «Пульт роликов» (`automontage pult`): русский главный экран со всеми роликами из `projects/`
  ...
  что и Review, и кнопка «Скопировать для агента». Перед утверждением пульт сверяет байты
  показанного preview, а черновик с невыбранным B-roll сначала отправляет в проверку монтажа.
  `automontage inbox` передаёт агенту правки и утверждённые ролики без финала. Нажатие
  «Утверждаю» на архивной карточке само возвращает её из архива – само утверждение и есть
  просьба пользователя собрать финал, поэтому во входящих остаётся обычная строка
  «Утверждено: …». Пометка «в архиве – не начинай без просьбы пользователя» появляется, если
  карточку убрали в архив уже после утверждения либо утверждение прошло не через эту кнопку
  (Review Workbench, чат с агентом); такая карточка честно показывает «Утверждено, в архиве –
  агент соберёт финал по вашей просьбе» и в карточке, и в подписи плеера, а новые правки того
  же ролика показываются как обычно.
  `automontage pult --install-shortcut` создаёт значок для macOS и Windows; окно открывается в
  Chrome/Edge в режиме приложения и работает без интернета. Интерфейс использует фирменный
  шрифт Onest, поставляемый вместе с движком, – без обращения в интернет за шрифтами.
- Запуск внешних инструментов (`scripts/process.js`) принимает необязательный `timeout`:
  зависший процесс завершается через `SIGKILL`. Пульт ограничивает им ffprobe/ffmpeg
  15 секундами; вызовы без опции работают как прежде.
- Все монтажные маршруты в новом чате начинают с одной карточки выбора: уникальный
  Creative Motion, готовый установленный стиль или авторский монтаж по референсу. После выбора
  агент автономно принимает обычные решения по типографике, графике, B-roll, музыке и таймингам,
  показывая пользователю один полный preview перед approved-only final.
- Creative Motion оформлен как project-local режим без нового клиентского кода в общем движке:
  новый арт-дирекшн, неповторяющиеся композиции, современные кириллические шрифты, реальные
  экраны и логотипы, поэтапные схемы и anti-template QA рендерятся в проверенный media/B-roll слой
  с глобальным таймкодом и мастер-аудио исходника.
- Новый публичный навык `reel-from-donor`: берёт разговорный Reels только как источник
  исследования, строит карту фактов, обязательно согласует CTA/дизайн/формат, показывает
  живые конкретные тексты карточек до графики и ведёт к preview/final через существующие
  монтажные маршруты. Включены контентный контракт, QA-чек-лист, публичная методология и
  адаптеры для Claude Code, Codex и общих agents.
- Safe-zone в карточечных и batch-инструкциях теперь проверяется на каждом кадре анимации,
  а не только после её остановки: текст, плашки и подчёркивания не могут выходить за край
  вертикального кадра через отрицательную координату или `translateX`.

### Исправлено

- Review Workbench закрывает файл, когда браузер обрывает загрузку (например, при перемотке
  видео). Раньше каждый такой обрыв оставлял открытый дескриптор до остановки сервера, и за
  долгую сессию он мог упереться в лимит открытых файлов.
- Транзитивный `js-yaml` обновлён до 4.3.2, чтобы закрыть high-severity DoS-уязвимость
  и снова пропускать обязательный security-gate CI без несовместимых обновлений зависимостей.
