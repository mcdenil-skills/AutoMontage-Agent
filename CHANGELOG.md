...
- «Пульт роликов» (`automontage pult`): русский главный экран со всеми роликами из `projects/`
  ...
  «Утверждено: …». Пометка «в архиве – не начинай без просьбы пользователя» появляется только
  для ролика, чью карточку убрали в архив уже после утверждения; такая карточка честно
  показывает «Утверждено, в архиве – агент соберёт финал по вашей просьбе», а новые правки
  того же ролика показываются как обычно.
