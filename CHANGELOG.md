...
- `automontage master` для дублей сам ищет паузу в звуке не дальше 0.25 с от каждой границы куска
  и ставит разрез в неё, печатает итоговые куски, время стыков и границы без паузы рядом;
  слова-галлюцинации Whisper в тишине на краях кусков не попадают в транскрипт. Навык
  `reel-turnkey` собирает тем же маршрутом один ролик из разных записей по сценарию.
