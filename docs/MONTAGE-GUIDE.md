...
```mermaid
flowchart LR
  A[Передать видео агенту] --> R[Выбрать маршрут монтажа]
  R --> B[Получить draft]
  B --> C[Проверить preview и B-roll]
...
Если способ оформления не указан, агент сразу покажет карточку **«Как монтируем?»**:

- **Уникальный Creative Motion — рекомендуется:** новый дизайн, разные композиции, современная
  типографика, реальные экраны, B-roll и анимированные схемы;
- **Готовый стиль:** выбор из реально установленных публичных и приватных тем;
- **По референсу:** авторская версия по механике присланного ролика без копирования его материалов.

После одного выбора агент сам решает обычные вопросы цвета, шрифта, ритма, переходов, B-roll и
музыки. Следующая обязательная точка пользователя — посмотреть полный preview и прислать правки
одним списком либо явно утвердить его перед final.
...
Подробно, с примерами просьб и требованиями к записям: [TAKES.md](TAKES.md).
