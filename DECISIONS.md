...
Карточку, убранную в архив уже после утверждения (обратный порядок действий), это не меняет:
`buildCards` показывает на ней честную надпись «Утверждено, в архиве – агент соберёт финал по
вашей просьбе» вместо обычной «Утверждено – агент собирает финал», и агент ждёт отдельной
просьбы, как и раньше.
