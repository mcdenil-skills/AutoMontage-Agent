...
Дубли обязаны совпадать по FPS и размеру кадра с учётом поворота и иметь звук: иначе FFmpeg либо
падает, либо молча выдаёт файл с переменным FPS. Звук приводится через `aformat` к самому полному
