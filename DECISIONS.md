...
(с точностью до кадра), граница уже в паузе остаётся на месте, и разрез, если пауза позволяет, не
ближе 40 мс к речи; в короткой паузе отступ получает речь, которая остаётся в куске. Сторону паузы
выбирает сам звук: разрез уходит через меньшую часть звучания, на которое попала граница. Стык
...
короткое «да» или возвращал вырезанное «эм». Граница без доступной паузы остаётся на месте и
печатается. Слова в тишине на краях куска считаются галлюцинациями Whisper и в транскрипт не
попадают. Звук для уровней и для Whisper читается с первой дорожки, как у `trim`, а видео остаётся в
пустом выводе: иначе у MPEG-TS с поздним звуком FFmpeg пересчитывает начало по звуку и все паузы и
слова съезжают. Для границ отклонены инструкция агенту без проверки звука (агент не слышит звук,
ручная проверка трудоёмка), повторная расшифровка master (обрывки не ловит) и постоянная сторона
паузы по типу края (после финального ревью её заменило правило середины).
