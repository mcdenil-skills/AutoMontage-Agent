...
повторная расшифровка master этого не заметила. Поэтому master сам ставит каждую границу в паузу
рядом по уровню звука дубля: окна по 10 мс; тишина не громче 15 дБ над шумом дубля или на 35 дБ
тише его речи, но всегда хотя бы на 20 дБ тише громкой речи и не выше -30 dBFS; пауза не короче 50
мс; поиск не дальше 0.25 с; граница в звуке уходит в паузу так, чтобы у края куска осталось около
0.1 с тишины (с точностью до кадра), граница уже в паузе остаётся на месте, и разрез, если пауза
позволяет, не ближе 40 мс к речи; в короткой паузе разрез встаёт ближе к её середине. Конец куска
предпочитает паузу до себя, начало после себя, потому что Whisper растягивает слова в паузы. Стык
...
который продолжается и по другую сторону границы (хвост или начало слова, внутрь которого Whisper
поставил границу), а граница на краю звука или в провале короче 50 мс остаётся на месте. Без этого
правила сдвиг молча терял короткое «да» или возвращал вырезанное «эм». Граница без доступной паузы
остаётся на месте и печатается. Слова в тишине на краях куска считаются галлюцинациями Whisper и в
транскрипт не попадают. Звук для уровней и для Whisper читается с первой дорожки, как у `trim`, а
видео остаётся в пустом выводе: иначе у MPEG-TS с поздним звуком FFmpeg пересчитывает начало по
звуку и все паузы и слова съезжают. Для границ отклонены инструкция агенту без проверки звука
(агент не слышит звук, ручная проверка трудоёмка) и повторная расшифровка master (обрывки не
ловит).
