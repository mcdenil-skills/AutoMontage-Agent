...
Уточнения после ревью. Имена папок проверяются одним правилом `scripts/pult/names.js`, которое
запрещает только опасное в пути: список разрешённых символов отклонял реальные legacy-папки
(NFD-кириллица macOS, скобки, плюс), и их нельзя было открыть. Перед утверждением пульт сам
сверяет байты показанного preview с паспортом, потому что для обычного lesson движок этот хеш
не проверяет; отказ движка при актуальном билете показывается отдельно (`APPROVAL_BLOCKED`), а
черновик с невыбранным B-roll вовсе не получает кнопки утверждения. Одновременный запуск двумя
кликами разводит замок `projects/.pult/starting.lock`, а проверка живого экземпляра отличает
занятый пульт (`busy`, например во время ffprobe) от отсутствующего (`absent`): занятый ждут
до 20 секунд, а не сразу запускают второй сервер, регистрацию отсутствующего снимают сразу.
