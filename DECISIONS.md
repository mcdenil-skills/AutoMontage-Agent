...
формату среди использованных дублей (наибольшая частота, стерео, если оно есть хотя бы у одного),
видео к CFR через `fps` до и после `trim`.

Уточнения по реальным данным. Whisper почти не оставляет пауз между словами (около 80% промежутков
нулевые), поэтому сводка режет фразы и по концу предложения, а запас на стыке ограничен половиной
паузы до соседа; слова нулевой длины получают 0.01 с. Probe дублей и master идёт по пути файла, а не
через `pipe:0` из D-017: по трубе ffprobe не читает длительность фрагментированного MP4 и MPEG-TS.
D-017 по-прежнему относится к Review Save и approval. Звук для Whisper извлекается с
`aresample=async=1:min_hard_comp=0:first_pts=0`, чтобы поздний старт звука не сдвигал слова раньше
оси `trim`.
