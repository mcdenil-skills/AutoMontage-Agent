...
до 20 секунд, а не сразу запускают второй сервер, регистрацию отсутствующего снимают сразу. Известное ограничение: если новая правка и
`inbox --accept` пишут `pult/comments.json` в одну и ту же миллисекунду, одна запись может
потеряться – межпроцессного замка у файла правок нет.

Уточнения после доводки пульта (архив и утверждение). Утверждение архивного ролика без финала
могло остаться незамеченным агентом: рассматривались вариант А – показывать такое утверждение
во входящих с пометкой «в архиве – не начинай без просьбы пользователя» и ждать отдельной
просьбы – и вариант Б – прятать эти утверждения из входящих вовсе, как и саму карточку. Выбран
А: пользователь видит, что утверждение не потеряно, а новые правки того же ролика по-прежнему
доходят до агента как обычно; Б рисковал тем, что утверждённый ролик навсегда остался бы без
финала, если пользователь просто забыл про архивную карточку.

Следующий круг этой же доводки вскрыл обратную проблему: архивная карточка всё равно показывала
кнопку «Утверждаю», и нажатие на неё оставляло карточку в архиве с той же пометкой «в архиве» –
явная просьба пользователя терялась, а final не собирался. Рассматривались вариант А –
утверждение само возвращает карточку из архива – и вариант Б – прятать кнопку «Утверждаю» на
архивной карточке вовсе, требуя сначала вернуть карточку из архива отдельным действием. Выбран
А: само нажатие «Утверждаю» и есть явная просьба пользователя собрать финал, значит одного
клика достаточно, без тупика «утверждено, но карточка выглядит так, будто её никто не заметил».

Правило для агента (ждать отдельной просьбы, пока карточка в архиве) это не меняет – меняется
только сама надпись, и вот почему. Карточку, убранную в архив уже ПОСЛЕ утверждения (обратный
порядок действий), это возвращение из архива не касается: она остаётся архивной. Прежняя
надпись status.js «Утверждено – агент собирает финал» на такой карточке вводила в заблуждение –
выглядело так, будто агент уже занят финалом, хотя по правилу AGENTS.md он ждёт отдельной
просьбы. Поэтому `buildCards` ставит на копии варианта флаг `archivedNeedsFinal` и заменяет эту
надпись на честную «Утверждено, в архиве – агент соберёт финал по вашей просьбе» (и подпись
плеера симметрично – «Утверждённый preview – в архиве, агент соберёт финал по вашей просьбе»).
Известное следствие: утверждение любого варианта архивной карточки темы (`group:<id>`)
возвращает из архива всю карточку, а не только утверждённый вариант – если один вариант темы
был утверждён и заархивирован раньше (с честной пометкой «в архиве»), а сейчас утверждают
другой вариант той же темы, пометка первого варианта исчезает вместе с общим возвратом
карточки, и агент по инструкции соберёт final для обоих вариантов, а не только для того, что
утвердили только что.

Известное ограничение: возврат из архива работает только при утверждении через кнопку
«Утверждаю» в самом пульте (`/api/approve`). Тот же `approveBrief`, вызванный не через эту
кнопку – в Review Workbench или явным «утверждаю» агенту в чате, – карточку из архива не
возвращает; она остаётся архивной с честной пометкой «в архиве» до отдельного действия
пользователя. Снятие этого ограничения – отдельная задача.
...
регистрируется в `project.json.takes`. Первым дублем становится исходный файл проекта; его
транскрипт создаётся заново, потому что активный `words.json` мог быть пересчитан `--tighten` или
master и больше не совпадает с оригиналом. Агент читает сводку фраз `automontage takes pack`,
выбирает лучший дубль для каждого смыслового блока и сохраняет `edit/vNN-takes.json`. `automontage
master` собирает выбранные куски одним FFmpeg filter graph в новую immutable source revision и
пересчитывает слова без повторного Whisper. Всё после master (draft, Review, preview, approval,
final) не меняется.

Границы кусков выравниваются по кадрам внутри master: начало вниз, конец вверх, с обрезкой по концу
более короткого потока дубля и не раньше начала более позднего потока; соседние куски одного дубля
делят общий кадр в пользу куска, который стоит раньше в списке. Кусок нецелого числа кадров удлиняет
видео до следующего кадра, а звук остаётся точным; на восьми кусках это давало расхождение 0.08 с.
Дубли обязаны совпадать по FPS, размеру кадра с учётом поворота и соотношению сторон пикселя и
иметь звук: иначе FFmpeg либо падает, либо пересчитывает частоту кадров, выбрасывая или повторяя
кадры. Звук приводится через `aformat` к самому полному
формату среди использованных дублей (наибольшая частота; stereo, если хотя бы у одного дубля два
канала и больше, иначе mono; многоканальный звук сводится в stereo), видео к CFR через `fps` до и
после `trim`.

Уточнения по реальным данным. Whisper почти не оставляет пауз между словами (около 80% промежутков
нулевые), поэтому сводка режет фразы и по концу предложения, а сводка и бриф выбора ограничивают
запас на стыке половиной паузы до соседа (master это не проверяет; `end` может выйти за конец дубля
не более чем на 0.25 с и обрезается); слова нулевой длины получают 0.01 с. Probe дублей и master
идёт по пути файла, а не через `pipe:0` из D-017: по трубе ffprobe не читает длительность
фрагментированного MP4 и MPEG-TS. D-017 по-прежнему относится к Review Save и approval. Звук для
Whisper извлекается с `aresample=async=1:min_hard_comp=0:first_pts=0`, чтобы поздний старт звука не
сдвигал слова раньше оси `trim`. Обычный source-edit master берёт из `probeMediaPath()` только
размер и поворот, поэтому ему разрешён opt-in `containerDurationFallback` для потоков без
собственной длительности; это узкое исключение из D-018. Дубли его не используют: их длительность
берётся по самому короткому потоку, и файл без длительности потока отклоняется.

Границы кусков по таймингам Whisper на живой пробе попали внутрь слова в 3 случаях из 8, и повторная
расшифровка master этого не заметила. Поэтому master сам ставит каждую границу в паузу рядом по
уровню звука дубля: окна по 10 мс; тишина не громче 15 дБ над шумом дубля или на 35 дБ тише его
речи, но всегда хотя бы на 20 дБ тише громкой речи и не выше -30 dBFS; пауза не короче 50 мс; поиск
не дальше 0.25 с; граница в звуке уходит в паузу так, чтобы у края куска осталось около 0.1 с тишины
(с точностью до кадра), граница уже в паузе остаётся в этой паузе, и разрез, если пауза позволяет,
не ближе 40 мс к речи; в короткой паузе отступ получает речь, которая остаётся в куске. Сторону
паузы выбирает сам звук: разрез уходит через меньшую часть звучания, на которое попала граница. Стык
соседних кусков одного дубля (или зазор меньше кадра) – один общий разрез, иначе слово между двумя
паузами терялось. Разрез не переходит к паузе через другое слово: пересекать можно только звук,
который продолжается и по другую сторону границы и лежит там большей частью (правило середины, как у
слов на стыке; это хвост или начало слова, внутрь которого Whisper поставил границу), а граница на
краю звука или в провале короче 50 мс остаётся на месте. Без этого правила сдвиг молча терял
короткое «да» или возвращал вырезанное «эм». Звук склеенных без паузы слов уровни не делят, поэтому
разрез не пересекает и середину слова Whisper: иначе терялось короткое «Не» перед «работает».
Граница без доступной паузы остаётся на месте и печатается. Слова в тишине на краях куска считаются
галлюцинациями Whisper и в транскрипт не попадают. Звук для уровней и для Whisper читается с первой
дорожки, как у `trim`, а видео остаётся в пустом выводе: иначе у MPEG-TS с поздним звуком FFmpeg
пересчитывает начало по звуку и все паузы и слова съезжают. Для границ отклонены инструкция агенту
без проверки звука (агент не слышит звук, ручная проверка трудоёмка), повторная расшифровка master
(обрывки не ловит) и постоянная сторона паузы по типу края (после финального ревью её заменило
правило середины).

Отклонены: склейщик по образцу browser-use/video-use (отдельное извлечение каждого куска и `-c copy`
concat), потому что в их открытых PR #62, #161 и #162 он накапливает рассинхрон и даёт щелчки AAC на
стыках; расширение `source-edit.schema.json`, потому что его диапазоны принадлежат одной активной
ревизии и требуют точных кадров от автора; облачная расшифровка, потому что она противоречит
локальной политике проекта. Смешение HDR и SDR дублей в первой версии не проверяется: общий probe не
читает `color_transfer`.

## D-032 – Палитра `--autotheme` без node-vibrant: пиксели ffmpeg и квантизатор MCU

**Дата:** 2026-09-26
**Статус:** принято

`node-vibrant@4.0.4` тянул `file-type@16.5.4` с зависанием на испорченном ASF
(GHSA-5v7r-6r5c-r473), а исправленного `node-vibrant` нет. Палитра теперь получает до 20
характерных кадров (`thumbnail=100`, `-fps_mode passthrough`) сразу сырыми rgb24-пикселями в
рамке 320×320 и квантует их `QuantizerCelebi` (64 цвета) из уже подключённой
`@material/material-color-utilities`. Взвешенное HCT-усреднение seed и сборка токенов не
изменились; новых зависимостей нет, JS-декодера картинок и временных файлов больше нет, объём
пикселей ограничен до разбора. Окна покрывают первые 2000 кадров ролика; равномерная выборка
по всей длине – отдельная задача.

Отвергнуто: продлевать временное исключение каждый месяц; `node-vibrant` 3.x (та же линия
Jimp); `overrides` на `file-type` 21 (ESM-only, ломает Jimp 0.22); `Score` для seed (hue уходит к
акцентам, а при пустом отборе подставляется запасной синий). Проверку исключения в
`check-release.js` удалили, а не оставили спящей: она была жёстко привязана к одной цепочке, а от
возврата уязвимости защищает `tests/palette-security.test.js`.

На реальном ролике seed сместился с hue 80.9 / chroma 14.0 до 92.2 / 13.6: часть сдвига дала
смена квантизатора, часть – исправленная выборка кадров (20 окон по 100 кадров вместо повтора
первого). При `--brandLock 1` (по умолчанию) все восемь цветовых токенов совпали побайтно,
при 0.5 и 0 они сдвинулись на 1–5 единиц канала, контраст текста остался не ниже 5.6:1.
