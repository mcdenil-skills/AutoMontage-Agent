...
- Утверждение проходит три шага. HMAC-билет `approvalTicket` от ключа, пути brief и SHA-256
  ...
  (обратный порядок действий), `buildCards` (`scripts/pult/cards.js`) заменяет обычную
  надпись status.js `Утверждено – агент собирает финал` (needsFinal без новых правок) на
  честную `Утверждено, в архиве – агент соберёт финал по вашей просьбе`, не трогая сами
  entries каталога; то же условие переключает подпись плеера в `pult/app.js`.
