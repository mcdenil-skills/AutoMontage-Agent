...
- Страница пульта (`pult/index.html`, `app.js`, `styles.css`) и единственный файл шрифта
  ...
  токена: каждый каталог на пути к файлу и сам файл проверяются на симлинк тем же способом, что
  и остальные защищённые файлы. Шрифт офлайновый и того же источника, поэтому `default-src
