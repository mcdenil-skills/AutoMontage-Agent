...
- Статус вычисляет чистая функция `scripts/pult/status.js` из `project.json`. «Готов» – только
  ...
  Legacy-папки получают статус из `pult-card.json`.
- Имена папок проверяет одно правило `scripts/pult/names.js` (`SAFE_NAME`, `isSafeName`):
  запрещено только опасное в пути (`/`, `\`, управляющие символы, имя с точки, больше 255
...
- `scanProjects` сканирует каждую папку отдельно: ошибка одной папки переводит её в
  «Не читается», а не роняет весь список. Маршруты медиа и действий ищут вариант точечно,
  `scanFolder(projectsDir, folderFromKey(key))`, без полного скана каталога на каждую обложку.
...
- `automontage inbox` (`scripts/pult/inbox.js`) читает тот же каталог: новые правки с секундой,
  ...
  `--accept <папка> <id>` отмечает правку принятой. Управляющие символы из текста правок
  вырезаются до печати в терминал.
