...
В новом чате пришли исходник и напиши «смонтируй». Агент сразу покажет три варианта:
**Уникальный Creative Motion** с новым дизайном, **Готовый стиль** из установленных тем или
**По референсу**. После одного выбора агент автономно делает транскрипт, режиссуру, современную
типографику, анимации, реальные экраны, B-roll, музыку, полный preview и QA. Перед final остаётся
только явно утвердить просмотренную версию.
...
ставит разрез в неё, потому что Whisper прячет паузы внутрь слов, и печатает итоговые куски,
время стыков и границы без паузы рядом. Тем же маршрутом можно собрать один ролик из разных
записей по сценарию: каждая запись становится дублем. Границы выравниваются по кадрам
автоматически; дубли должны совпадать по FPS, размеру кадра и соотношению сторон пикселя и
иметь звук. Дальше маршрут тот же, но draft, Review, preview, утверждение и final работают с
новой source revision: в `source` draft указывай абсолютный путь к активному
...
3. Если маршрут не указан, агент покажет карточку: уникальный Creative Motion, готовый стиль
   или монтаж по референсу.
4. После выбора агент автономно соберёт полный preview. Пришли правки одним списком или явно
   утверди его — тогда агент выпустит и проверит final.
...
Не хочешь ничего расписывать — просто напиши «смонтируй». Если добавишь «всё реши сам», агент
автоматически выберет рекомендуемый Creative Motion и не будет спрашивать про каждый цвет,
шрифт, переход или музыкальный параметр.
